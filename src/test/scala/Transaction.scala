...
class Transaction extends FunSuite {
  ...
  test("transaction") {
    ...
    def insertCity(id: Long, name: String, countrycode: String, district: String, population: Int): Update0 = sql"insert into city (id, name, countrycode,district,population) values ($id, $name, $countrycode,$district,$population)".update

    //duplicate key value violates unique constraint "city_pkey". No records insert!
    val o = transactor.use { xa =>
      for {
